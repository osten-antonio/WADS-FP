// Statistical distribution lookup tables.
// Mirrors frontend/lib/statistics/tables.ts but only includes the lookup
// functions and data required by the backend calculation engine (math.ts).
// The frontend keeps its own copy because the Reference tool renders the
// tables visually and still runs client-side.
...
// F-Distribution Critical Values (right-tail). Used in ANOVA and F-tests.
