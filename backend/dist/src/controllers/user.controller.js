...
import { deleteUserHistory, getUserById, getUserHistory, getUserProfile, syncUserAccount, updateUserDisplayName, } from "../services/user.service";
...
function getFallbackDisplayName(decoded) {
    return decoded.name?.trim() || decoded.email?.split("@")[0] || decoded.uid;
}
function getAuthenticatedUserId(res) {
...
export async function filterHistory(req, res) {
    try {
        const userId = getAuthenticatedUserId(res);
        if (!userId) {
...
export async function deleteHistory(req, res) {
    try {
        ...
        const pathSubmissionId = typeof req.params.id === "string" && req.params.id.trim().length > 0 ? req.params.id.trim() : undefined;
        let submissionIds;
        if (pathSubmissionId) {
