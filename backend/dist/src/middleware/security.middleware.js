...
const IMAGE_MIME_TO_SIGNATURE_TYPE = {
    "image/jpeg": "jpeg",
    "image/jpg": "jpeg",
    "image/png": "png",
    "image/webp": "webp",
...
export function normalizeAndCleanText(value) {
...
function containsEncodedInjection(value) {
...
export function containsPromptInjectionIndicators(value) {
...
function hasUnsafeObjectKeys(value, seen = new WeakSet()) {
...
function fail(res, message, code, status = 400, details) {
    return res.status(status).json({ message, code, details });
}
function sanitizeStringField(options) {
...
export function validateIngestionTextSecurity(req, res, next) {
...
export function validateGenerateExplanationSecurity(req, res, next) {
    if (!validateBodyObject(req, res))
        return;
    const question = sanitizeStringField({
        field: "question",
    ...
    const stepObject = req.body.step;
...
export function validateFollowUpSecurity(req, res, next) {
    if (!validateBodyObject(req, res))
        return;
...
export function isSafeDisplayName(displayName) {
    return SAFE_DISPLAY_NAME_REGEX.test(displayName);
}
export function validateUpdateUsernameSecurity(req, res, next) {
    if (!validateBodyObject(req, res))
        return;
    ...
    if (!isSafeDisplayName(displayName.value)) {
        fail(res, "displayName contains unsupported characters", "SECURITY_INVALID_DISPLAY_NAME");
        return;
    }
    req.body.displayName = displayName.value;
    next();
}
export function detectImageTypeFromSignature(buffer) {
...
export function validateImageUploadSecurity(req, res, next) {
    const file = req.file;
    if (!file) {
    ...
    if (!signatureType || !expectedType || signatureType !== expectedType) {
        fail(res, "Image signature does not match declared MIME type", "SECURITY_INVALID_FILE_SIGNATURE");
        return;
    }
    next();
}
