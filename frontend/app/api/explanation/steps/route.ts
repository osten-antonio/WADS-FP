...
export async function POST(req: NextRequest) {
  ...
  try {
    const backendResponse = await backendApi.post(`/explanation/steps/`, body);
    return NextResponse.json(backendResponse.data, { status: backendResponse.status });
  } catch (error) {
    if (axios.isAxiosError(error)) {
      if (error.code === "ECONNABORTED") {
        return NextResponse.json({ message: "Explanation request timed out, is AI service available?\n Contact project team if issue still persists" }, { status: 504 });
      }
      if (error.response) {
